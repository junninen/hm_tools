function [posSum,negSum,tim,dp,mob,posDat,posErr,negDat,negErr]=hm_load_ais_nd(fullPath2file)
%Load ais data that is saved in the new format after 31.07.2006 
%
...
%is the positive first or negative? stupid format...
negFirst=0;
if ~isempty(regexp(hdr{2},'neg'))
    negFirst=1;
end
...
if negFirst
    negDat=datRaw(:,1:27);
    negErr=datRaw(:,28:27*2);
    posDat=datRaw(:,55:27+55-1);
    posErr=datRaw(:,82:27+82-1);
else
    posDat=datRaw(:,1:27);
